...
farms_df['poa_tmy'] = pvlib.irradiance.get_total_irradiance(surface_tilt=tilt, surface_azimuth=azimuth,
                            solar_zenith=farms_df['zenith'], solar_azimuth=farms_df['azimuth'],
                            dni=tmy_data['DNI'], ghi=tmy_data['GHI'], dhi=tmy_data['DHI'],
                            dni_extra=farms_df['dni_extra'], airmass=farms_df['airmass'], albedo=tmy_data['Surface Albedo'],
...
for cloud in poa_nmbe.index:
    ...
    if cloud == 0:
        ...
        poa_nmbe['TMY'], poa_nrmse['TMY'] = stats_calcs(cloud_real, cloud_farms['poa_tmy'])
    else: 
        poa_nmbe['FARMS'].loc[cloud], poa_nrmse['FARMS'].loc[cloud] = stats_calcs(cloud_real, cloud_farms['poa_global'])
        poa_nmbe['TMY'].loc[cloud], poa_nrmse['TMY'].loc[cloud] = stats_calcs(cloud_real, cloud_farms['poa_tmy'])

poa_nmbe = poa_nmbe.dropna()
poa_nmbe = poa_nmbe.drop(columns='TMY')
poa_nrmse = poa_nrmse.dropna()
poa_nrmse = poa_nrmse.drop(columns='TMY')
...
#         all_poa_nmbe['TMY'], all_poa_nrmse['TMY'] = stats_calcs(cloud_real, cloud_farms['poa_tmy'])
#     else: 
#         all_poa_nmbe['FARMS'], all_poa_nrmse['FARMS'] = stats_calcs(cloud_real, cloud_farms['poa_global'])
#         all_poa_nmbe['TMY'], all_poa_nrmse['TMY'] = stats_calcs(cloud_real, cloud_farms['poa_tmy'])
